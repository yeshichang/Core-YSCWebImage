...
@implementation YSCWebImageWebPCoder {
    WebPIDecoder *_idec;
}
...
+ (instancetype)sharedCoder {
    static YSCWebImageWebPCoder *coder;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        coder = [[YSCWebImageWebPCoder alloc] init];
    });
    return coder;
}
...
- (BOOL)canDecodeFromData:(nullable NSData *)data {
    return ([NSData YSC_imageFormatForImageData:data] == YSCImageFormatWebP);
}
...
- (UIImage *)decodedImageWithData:(NSData *)data {
    ...
    CGContextRef canvas = CGBitmapContextCreate(NULL, canvasWidth, canvasHeight, 8, 0, YSCCGColorSpaceGetDeviceRGB(), bitmapInfo);
    if (!canvas) {
    ...
    if (!(flags & ANIMATION_FLAG)) {
        ...
        UIImage *staticImage = [self YSC_rawWebpImageWithData:webpData];
        if (staticImage) {
    ...
    do {
        @autoreleasepool {
            UIImage *image = [self YSC_drawnWebpImageWithCanvas:canvas iterator:iter];
            if (!image) {
                continue;
            }
            ...
            YSCWebImageFrame *frame = [YSCWebImageFrame frameWithImage:image duration:duration / 1000.f];
    ...
    UIImage *animatedImage = [YSCWebImageCoderHelper animatedImageWithFrames:frames];
...
- (UIImage *)incrementallyDecodedImageWithData:(NSData *)data finished:(BOOL)finished {
    ...
    if (width + height > 0 && last_y > 0 && height >= last_y) {
        ...
        CGColorSpaceRef colorSpaceRef = YSCCGColorSpaceGetDeviceRGB();
        ...
        CGContextRef canvas = CGBitmapContextCreate(NULL, width, height, 8, 0, YSCCGColorSpaceGetDeviceRGB(), bitmapInfo);
        if (!canvas) {
...
- (nullable UIImage *)YSC_drawnWebpImageWithCanvas:(CGContextRef)canvas iterator:(WebPIterator)iter {
    UIImage *image = [self YSC_rawWebpImageWithData:iter.fragment];
    if (!image) {
        return nil;
    }
...
- (nullable UIImage *)YSC_rawWebpImageWithData:(WebPData)webpData {
...
- (BOOL)canEncodeToFormat:(YSCImageFormat)format {
    return (format == YSCImageFormatWebP);
}
...
- (NSData *)encodedDataWithImage:(UIImage *)image format:(YSCImageFormat)format {
    ...
    if (frames.count == 0) {
        // for static single webp image
        data = [self YSC_encodedWebpDataWithImage:image];
    } else {
        // for animated webp image
        ...
        for (size_t i = 0; i < frames.count; i++) {
            YSCWebImageFrame *currentFrame = frames[i];
            NSData *webpData = [self YSC_encodedWebpDataWithImage:currentFrame.image];
        ...
        int loopCount = (int)image.YSC_imageLoopCount;
...
- (nullable NSData *)YSC_encodedWebpDataWithImage:(nullable UIImage *)image {
    if (!image) {
        return nil;
    }
